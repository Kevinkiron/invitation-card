/* ══════════════════════════════════════════════════════════════════════
   ...
   The premium template for the three celebrations that are not weddings:
   a birthday, a naming ceremony / baby shower, and a housewarming
   (griha pravesh).

   One schema, three kinds. `_premium` carries the kind AND acts as the
...
/* The three kinds we render. Anything else is a caller mistake, and a
   mistake that silently becomes a birthday is easier to ship than one
   that throws — but only if it is corrected here rather than guessed at
   in four different components downstream. */
export const CELEBRATION_KINDS = ["birthday", "naming", "housewarming"];

const asKind = (k) => (CELEBRATION_KINDS.includes(k) ? k : "birthday");
...
export function emptyCelebrationTokens(kind) {
  ...
  return {
    ...
    palette: { ...CELEBRATION_PALETTES[k] },
  };
}
...
export function patchCelebrationTokens(current, patch) {
  ...
  const kept = CELEBRATION_KINDS.includes(current?._premium) ? current._premium : null;
...
export const CELEBRATION_SCHEMA = {
  ...
  properties: {
    ...
    eventKind: { type: "string", description: "One of: birthday, naming, housewarming." },
    ...
    host: {
      ...
      properties: {
        ...
        role:        { type: "string", description: "e.g. 'the birthday boy', 'our little one', 'our new home'." },
