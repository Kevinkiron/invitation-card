...
export default function CelebrationCinema({ tokens: rawTokens, preview = false }) {
  ...
  const music = useCinemaMusic(kind, tokens, { preview });
  const openInvitation = useCallback(() => {
    setOpened(true);
    /* Same click, same tick — the one real user gesture the browser will
       accept as permission to start audio with sound. */
