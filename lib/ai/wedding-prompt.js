...
export const WEDDING_STEPS = [
  ...
  {
    /* Each card in the celebrations timeline (see EventCard in
       WeddingCinema.js) shows event.imageUrl when there is one and a
       plain initial otherwise — nothing before this step ever asked for
       one, so every function card showed the fallback letter no matter
       how much else about the wedding was filled in.

       Same shape as storyPhotos: one paperclip action, several pictures
       at once, matched to slots in order (lib/photos.js's "events"
       slot fills events[].imageUrl in array order). Asking one function
       at a time does not work here — `ack: true` steps are marked
       answered after a single turn (see ackWeddingStep in
       app/api/ai/design/route.js), so a couple sending one photo per
       turn would only ever get the first card filled in before the
       interview moved on. */
    id: "functionPhotos",
    group: "The celebrations",
    ack: true,
    unlocks: "a photograph on each card in the celebrations timeline",
    done: (t) =>
      list(t?.events).length > 0 &&
      list(t?.events).filter((e) => e.imageUrl).length >= list(t?.events).length,
    ask: (t) => {
      const names = list(t?.events).map((e) => e.name).filter(Boolean);
      return `Now a photograph for each of the celebrations — ${names.join(
        ", "
      )} — tap the paperclip and send them together, in that order. Or say skip and I will leave them plain.`;
    },
  },
