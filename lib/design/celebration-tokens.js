...
export const CELEBRATION_SCHEMA = {
  ...
  properties: {
    ...
    askField: {
      type: "string",
      enum: ["text", "date", "datetime"],
      description:
        "What kind of answer askNext is waiting for, so the interface can show " +
        "the right control instead of a plain text box. Use 'datetime' when " +
        "askNext is asking for the event's own date (guests will also need a " +
        "start time). Use 'date' for any OTHER date-only question — an RSVP " +
        "deadline, for instance. Use 'text' for everything else, including " +
        "when askNext is empty because the interview is done.",
    },
