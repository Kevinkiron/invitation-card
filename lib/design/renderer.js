...
export function render(tok, mount) {
  ...
  /* The ground the sheet floats on. An invitation is a piece of stationery,
     not a website: it should be the same narrow column on a laptop as on a
     phone, and the space either side has to read as the table it is lying
     on rather than as empty page. Light palettes get a slightly deeper wash
     of their own paper; dark ones go darker still, because lightening the
     surround would turn the sheet into a hole. */
  const ground = lum(c.bg) > 0.5 ? mix(c.bg, c.ink, 0.1) : mix(c.bg, "#000000", 0.45);
  ...
  mount.innerHTML = `
    <style>
      .inv-root{
        ...
        --ground:${ground};
        /* The sheet width. Every measure on the page derives from this, so
           a laptop gets the same invitation a phone does — larger type and
           more air, not a different layout. */
        --sheet:${d.sheet || "600px"};
        /* The measure. Every size on the page is a fraction of THIS, not of
           the viewport — otherwise a laptop got the same 44px names inside a
           600px sheet that a phone got inside a 390px one, and the desktop
           invitation read as a phone screenshot pasted onto a big screen. */
        --m:min(100vw, var(--sheet));
        background:var(--ground); color:var(--ink); font-family:var(--body); font-weight:300;
        position:relative; overflow-x:hidden;
        text-rendering:optimizeLegibility; -webkit-font-smoothing:antialiased;
      }

      /* ── the sheet ──
         Everything lives in one centred column. Before this the hero
         stretched the full 1440px of a laptop and a 300px portrait floated
         in the middle of it with the couple's names set smaller than their
         own photograph. A printed invitation is never that shape. */
      .inv-root .sheet{
        position:relative; z-index:1;
        max-width:var(--sheet); margin:0 auto; background:var(--bg);
        box-shadow:0 0 90px -26px rgba(0,0,0,.30);
      }
      ...
      .inv-root .hero .deco{position:absolute;background-image:var(--motif);background-size:contain;background-repeat:no-repeat;
        width:clamp(96px,calc(var(--m)*.24),190px);height:clamp(96px,calc(var(--m)*.24),190px);
      ...
      .inv-root .epigraph{max-width:520px;z-index:2;margin-bottom:20px;font-family:var(--display);font-style:italic;
        font-size:clamp(14px,calc(var(--m)*.028),18px);line-height:1.75;color:var(--muted)}
      ...
      /* A photograph earns a little more room than an empty placeholder,
         but neither is allowed to out-shout the names above it. At the old
         300px the portrait was over half the width of a phone screen and
         the first thing anyone saw. */
      .inv-root .hero.hasphoto .frame{width:clamp(132px,calc(var(--m)*.30),178px)}
      .inv-root .frame{position:relative;z-index:2;width:clamp(118px,calc(var(--m)*.26),158px);aspect-ratio:3/4;overflow:hidden;
        margin-top:34px;border:1px solid var(--accent-soft);background:var(--surface);
        box-shadow:0 16px 34px -16px rgba(0,0,0,.30), 0 0 0 5px var(--bg), 0 0 0 6px var(--accent-soft)}
      ...
      .inv-root .frame.wide{border-radius:var(--radius);width:clamp(190px,calc(var(--m)*.50),300px);aspect-ratio:16/9}
      ...
      .inv-root .kicker{z-index:2;font-size:clamp(10px,calc(var(--m)*.019),12px);letter-spacing:.32em;
      ...
      .inv-root .h1{z-index:2;font-family:var(--display);font-weight:${d.displayWeight || 600};
        font-size:clamp(30px,calc(var(--m)*${(d.h1Scale || 7) / 100}),${d.h1Max || 72}px);line-height:1.12;color:var(--headline);
        text-transform:${d.displayCase || "none"};letter-spacing:${d.displayTracking || "0"}}
      /* Script faces have a tiny x-height for their point size — set at the
         same number as a serif they read half as large. The names are the
         invitation, so they get their own scale rather than the token's. */
      .inv-root .h1.scriptface{font-family:var(--script);font-weight:400;text-transform:none;letter-spacing:0;
        font-size:clamp(40px,calc(var(--m)*.125),74px);line-height:1.06}
      ...
      .inv-root .subhead{z-index:2;margin-top:16px;font-family:var(--display);font-size:clamp(16px,calc(var(--m)*.037),23px);color:var(--ink)}
      ...
      .inv-root .cd{display:flex;align-items:flex-start;justify-content:center;gap:clamp(10px,calc(var(--m)*.036),22px);
        margin-top:34px;padding-top:22px;border-top:1px solid var(--accent-soft);z-index:2;
        width:min(420px,92%)}
      .inv-root .cd > div{min-width:52px}
      .inv-root .cd .n{font-family:var(--display);font-size:clamp(26px,calc(var(--m)*.064),40px);
      ...
      .inv-root .cd .sep{min-width:0;font-family:var(--display);font-size:clamp(20px,calc(var(--m)*.048),30px);
        color:var(--accent-soft);line-height:1.15}

      /* ── sections ── */
      .inv-root .sec{position:relative;padding:clamp(56px,9vw,82px) clamp(22px,6vw,54px);overflow:hidden}
      ...
      .inv-root .sec .deco{position:absolute;background-image:var(--motif);background-size:contain;background-repeat:no-repeat;
        width:clamp(110px,calc(var(--m)*.22),190px);height:clamp(110px,calc(var(--m)*.22),190px);opacity:calc(var(--density)*.3);pointer-events:none}
      ...
      /* The sheet already sets the measure. This used to be 1000px, which
         on a laptop stretched a two-item schedule across the screen. */
      .inv-root .wrap{max-width:100%;margin:0 auto;position:relative;z-index:2}
      ...
      .inv-root .eyebrow i{width:clamp(18px,calc(var(--m)*.06),44px);height:1px;background:var(--accent-soft)}
      ...
      .inv-root .h2{text-align:center;font-family:var(--display);font-weight:${d.displayWeight || 600};
        font-size:clamp(26px,calc(var(--m)*.064),42px);color:var(--headline);text-transform:${d.displayCase || "none"};letter-spacing:${d.displayTracking || "0"}}
      .inv-root .h2.scriptface{font-family:var(--script);font-weight:400;text-transform:none;letter-spacing:0;font-size:clamp(30px,calc(var(--m)*.076),50px)}
      ...
      .inv-root .grid{display:grid;grid-template-columns:repeat(var(--gcols,3),1fr);
        grid-auto-rows:clamp(112px,calc(var(--m)*.26),152px);grid-auto-flow:dense;gap:12px;margin-top:36px;
      ...
      .inv-root .grid[style*="--gcols:1"]{max-width:230px;grid-auto-rows:300px}
      .inv-root .grid[style*="--gcols:2"]{max-width:360px}
      ...
      /* The gallery keeps its columns on a phone. It used to collapse to a
         single 230px-tall full-width tile per photo, which turned an album
         into a stack of banners — the same mistake the hero portrait was
         making. The sheet is the same measure on every screen, so the grid
         can be too; two columns is the narrowest it ever needs to go. */
      @media(max-width:640px){
        .inv-root .grid{grid-template-columns:repeat(2,1fr)}
        .inv-root .grid[style*="--gcols:1"]{grid-template-columns:1fr;max-width:200px;grid-auto-rows:260px}
        .inv-root .sec{padding:56px 20px}
    ...
    <div class="inv-root">
      ...
      <div class="hero${ct.heroPhoto ? " hasphoto" : ""}">
        ...
        <div class="heroin">
          ...
          <!-- The portrait sits BELOW the names on purpose. When it led the
               hero it was the first and largest thing on the invitation and
               the couple's own names were set smaller than their photo. The
               names are the invitation; the photograph illustrates it. -->
          ${d.frame !== "none" ? `
