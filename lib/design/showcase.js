...
export const EVENTS = [
  { slug: "wedding",      name: "Wedding",         tag: "Two hearts, one journey",      icon: "church", palette: ["#fdf9f1", "#b8912f", "#2b4433"], motif: "botanical", image: "/images/cards/wedding.jpg" },
  { slug: "birthday",     name: "Birthday",        tag: "Another year worth marking",   icon: "cake",   palette: ["#fdf6ee", "#c26b3c", "#5c3a24"], motif: "rings", image: "/images/cards/birthday.jpg" },
  { slug: "naming",       name: "Naming Ceremony", tag: "A name given with love",       icon: "star",   palette: ["#fff9f4", "#d69a5c", "#5e4028"], motif: "rings", image: "/images/cards/naming.jpg" },
  { slug: "housewarming", name: "Housewarming",    tag: "Beginning life in a new home", icon: "home",   palette: ["#f6f7f2", "#7a8b5a", "#33402b"], motif: "botanical", image: "/images/cards/housewarming.jpg" },
];
...
/* Card artwork. Small parametric SVGs tinted from each event's accent —
   ...
   Each EVENTS entry also carries a real placeholder photo (`image`,
   under public/images/cards/) that sits underneath this line art as the
   card's actual picture; see .w-ethumb in landing.css and app/page.js. */
  ...
  const g =
    {
      ...
      /* ── Greeting-card motifs (lib/greetings/occasions.js) ──
         Same parametric-SVG approach as the six above: geometry tinted
         from the occasion's accent, no bespoke artwork to commission. */

      /* Christmas — a sprig of holly (two leaves, three berries),
         scattered like the confetti motif above. */
      holly: Array.from({ length: 7 }, (_, i) => {
        const x = 20 + ((i * 53) % 165), y = 18 + ((i * 71) % 168), r = (i * 41) % 40 - 20;
        return `<g transform="translate(${x} ${y}) rotate(${r})" opacity=".5">
          <path d="M0 0 C-10 -4 -16 -14 -10 -22 C-4 -16 0 -8 0 0 Z" fill="${accent}"/>
          <path d="M0 0 C10 -4 16 -14 10 -22 C4 -16 0 -8 0 0 Z" fill="${accent}"/>
          <circle cx="-4" cy="4" r="2.6" fill="${accent}"/>
          <circle cx="4" cy="4" r="2.6" fill="${accent}"/>
          <circle cx="0" cy="7" r="2.6" fill="${accent}"/>
        </g>`;
      }).join(""),

      /* New Year / Congratulations — bursts radiating from a few points,
         each a ring of short rays plus a soft core. */
      firework: [[50, 60], [150, 90], [100, 150]].map(([cx, cy], gi) => {
        const rays = Array.from({ length: 10 }, (_, i) => {
          const a = (i / 10) * Math.PI * 2;
          const r1 = 6, r2 = 15 + (gi % 2) * 5;
          return `<line x1="${cx + Math.cos(a) * r1}" y1="${cy + Math.sin(a) * r1}" x2="${cx + Math.cos(a) * r2}" y2="${cy + Math.sin(a) * r2}" stroke="${accent}" stroke-width="1.4" stroke-linecap="round" opacity=".6"/>`;
        }).join("");
        return `<g>${rays}<circle cx="${cx}" cy="${cy}" r="3" fill="${accent}" opacity=".8"/></g>`;
      }).join(""),

      /* Diwali — a row of small oil-lamp silhouettes, each with a flame,
         plus a garland of marigold dots strung beneath them. */
      diya: Array.from({ length: 5 }, (_, i) => {
        const x = 24 + i * 38, y = 150;
        return `<g transform="translate(${x} ${y})" opacity=".55">
          <path d="M-16 0 Q0 16 16 0 Q0 8 -16 0 Z" fill="${accent}"/>
          <ellipse cx="0" cy="-9" rx="3" ry="7" fill="${accent}"/>
        </g>`;
      }).join("") + Array.from({ length: 9 }, (_, i) => {
        const x = 18 + i * 21, y = 118 + Math.sin(i) * 5;
        return `<circle cx="${x}" cy="${y}" r="2.4" fill="${accent}" opacity=".4"/>`;
      }).join(""),

      /* Onam — a pookalam (flower-carpet) rendered as concentric rings of
         small petals, like a rangoli seen from above. */
      pookalam: [78, 56, 34].map((r, ri) => {
        const n = 8 + ri * 4;
        return Array.from({ length: n }, (_, i) => {
          const a = (i / n) * Math.PI * 2;
          const x = 100 + Math.cos(a) * r, y = 100 + Math.sin(a) * r;
          return `<circle cx="${x}" cy="${y}" r="${5 - ri}" fill="${accent}" opacity="${0.55 - ri * 0.12}"/>`;
        }).join("");
      }).join("") + `<circle cx="100" cy="100" r="10" fill="${accent}" opacity=".5"/>`,

      /* Eid — a crescent moon and a small star, repeated. */
      crescent: [[52, 60], [148, 130]].map(([cx, cy], i) => `
        <g transform="translate(${cx} ${cy})" opacity=".55">
          <path d="M0 -20 A20 20 0 1 0 0 20 A15.5 15.5 0 1 1 0 -20 Z" fill="${accent}"/>
        </g>
        <path d="M${cx + 32} ${cy - 26} l2.6 6.4 6.4 1-4.8 4.4 1.2 6.4-5.4-3.3-5.4 3.3 1.2-6.4-4.8-4.4 6.4-1z" fill="${accent}" opacity=".5"/>
      `).join(""),

      /* Holi — loose rounded splashes at varied sizes, like thrown
         colour caught mid-air. */
      splash: Array.from({ length: 9 }, (_, i) => {
        const x = 16 + ((i * 61) % 172), y = 14 + ((i * 83) % 176), r = 8 + (i % 4) * 5;
        return `<circle cx="${x}" cy="${y}" r="${r}" fill="${accent}" opacity="${0.2 + (i % 3) * 0.12}"/>`;
      }).join(""),

      /* Raksha Bandhan — a simple bow (the rakhi thread), repeated. */
      thread: Array.from({ length: 4 }, (_, i) => {
        const x = 34 + i * 44, y = 40 + (i % 2) * 110;
        return `<g transform="translate(${x} ${y})" opacity=".5">
          <path d="M0 0 C-16 -12 -16 12 0 0 C16 -12 16 12 0 0 Z" fill="none" stroke="${accent}" stroke-width="2"/>
          <circle cx="0" cy="0" r="3" fill="${accent}"/>
        </g>`;
      }).join(""),

      /* Pongal — a sun with radiating triangular rays, rangoli-style. */
      sun: (() => {
        const rays = Array.from({ length: 16 }, (_, i) => {
          const a = (i / 16) * Math.PI * 2;
          const x1 = 100 + Math.cos(a) * 36, y1 = 100 + Math.sin(a) * 36;
          const x2 = 100 + Math.cos(a) * 60, y2 = 100 + Math.sin(a) * 60;
          return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${accent}" stroke-width="2" stroke-linecap="round" opacity=".45"/>`;
        }).join("");
        return `${rays}<circle cx="100" cy="100" r="30" fill="none" stroke="${accent}" stroke-width="2" opacity=".55"/>`;
      })(),
    }[motif] || "";
