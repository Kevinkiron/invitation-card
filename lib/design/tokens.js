...
export const TYPE_SETS = {
  "classic-script":  { display: "'Cormorant Garamond',Georgia,serif", body: "'Jost',sans-serif",          script: "'Pinyon Script',cursive",   weight: 600 },
  "editorial-serif": { display: "'Playfair Display',Georgia,serif",   body: "'DM Sans',sans-serif",       script: "'Playfair Display',serif",  weight: 700 },
  "modern-sans":     { display: "'Space Grotesk',sans-serif",         body: "'DM Sans',sans-serif",       script: "'Space Grotesk',sans-serif",weight: 700 },
  "poster-condensed":{ display: "'Bebas Neue',Impact,sans-serif",     body: "'Space Grotesk',sans-serif", script: "'Bebas Neue',sans-serif",   weight: 400 },
  "quiet-serif":     { display: "'Cormorant Garamond',Georgia,serif", body: "'DM Sans',sans-serif",       script: "'Cormorant Garamond',serif",weight: 600 },
};
...
export function sanitiseDesign(incoming = {}, current = {}) {
  ...
  if (incoming.typeSet && TYPE_SETS[incoming.typeSet]) {
    d.typeSet = incoming.typeSet;
    const t = TYPE_SETS[incoming.typeSet];
    d.fonts = { display: t.display, body: t.body, script: t.script };
    d.displayWeight = t.weight;
  }
  if (incoming.frame)  d.frame  = pick(incoming.frame,  FRAMES,  d.frame  || "arch");
  if (incoming.motif)  d.motif  = pick(incoming.motif,  MOTIFS,  d.motif  || "botanical");
  if (incoming.reveal) d.reveal = pick(incoming.reveal, REVEALS, d.reveal || "fade");
  if (incoming.corner) d.corner = pick(incoming.corner, CORNERS, d.corner || "soft");
