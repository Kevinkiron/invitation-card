...
export const DESIGN_SCHEMA = {
  ...
  properties: {
    ...
    content: {
      type: "object",
      description:
        "REQUIRED on EVERY turn, never omitted and never empty once you know anything. " +
        "This object IS the invitation — the page is built from it and from nothing else. " +
        "A fact you acknowledge in `reply` but do not write here is thrown away. " +
        "Repeat everything already established plus whatever is new: if you know the names, " +
        "send headline (and headlineB); if you know the date or time, send subhead; " +
        "if you know the venue, send place. Never invent any of them.",
      properties: {
        monogram:  { type: "string", description: "Two initials joined, e.g. 'A & D'. Only for personal events." },
        kicker:    { type: "string" },
        headline:  { type: "string", description: "The main name or title. Send it as soon as you know it, and send it again on every later turn." },
        ...
        subhead:   { type: "string", description: "Date and time, written out. Send it on every turn once known." },
        place:     { type: "string", description: "Venue and town. Send it on every turn once known." },
  ...
  /* `content` and `eventKind` are required because leaving them optional
     meant the model simply never sent them. It replied warmly, asked the
     right questions, acknowledged the names and the venue — and returned
     content:{} every turn, so published invitations carried no details at
     all. Requiring them is what forces the invitation to be written. */
  required: ["reply", "eventKind", "content"],
};
...
/* Photo sources are stricter than links: they end up in an <img src>, so
   only ordinary web images are allowed. `data:` is refused because a real
   photo as a data URI would bloat every row and every AI request; uploads
   go to storage and come back as https URLs. */
const safePhoto = (v) => {
  const s = str(v, 700);
  if (!s) return undefined;
  const t = s.trim();
  if (/^https:\/\//i.test(t)) return t;
  if (t.startsWith("/")) return t;   // same-site (e.g. /uploads/…)
  return undefined;
};

export function sanitisePhotos(list, max = 12) {
  if (!Array.isArray(list)) return [];
  return list.map(safePhoto).filter(Boolean).slice(0, max);
}

function sanitiseSection(s = {}, currentById = {}) {
  ...
  /* Photos belong to the person, not the model. Two rules:
       - every source is scheme-checked, because the old code passed the
         model's array straight into an <img src>;
       - a patch that omits `photos` keeps the ones already uploaded. The
         model rewrites this section on almost every turn, and without this
         a caption change silently deleted the customer's photographs. */
  if (s.type === "gallery") {
    const incoming = sanitisePhotos(s.photos);
    const kept = sanitisePhotos(currentById[out.id || "gallery"]?.photos);
    out.photos = incoming.length ? incoming : kept;
  }
  return out;
}
...
export function sanitiseContent(incoming = {}, current = {}) {
  ...
  /* heroPhoto and gallery photos are client-owned: uploaded by the person,
     written by us, never by the model. `c = { ...current }` already carries
     heroPhoto forward; this is the matching index so sanitiseSection can
     carry gallery photos forward too. */
  const currentSections = Array.isArray(current.sections) ? current.sections : [];
  const currentById = {};
  currentSections.forEach((sec) => {
    if (sec?.type === "gallery") currentById[sec.id || "gallery"] = sec;
  });

  if (Array.isArray(incoming.sections)) {
    const secs = incoming.sections.slice(0, 8).map((sec) => sanitiseSection(sec, currentById)).filter(Boolean);
