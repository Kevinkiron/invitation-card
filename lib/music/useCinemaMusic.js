...
import { resolveTrack, trackSrc } from "@/lib/music/tracks";
...
export function useCinemaMusic(kind, tokens, { preview = false } = {}) {
  /* `resolveTrack` prefers a track the couple/host chose or uploaded in
     the "Background music" picker (components/MusicPicker.js) over the
     deterministic automatic pick — see lib/music/tracks.js. */
  const track = useMemo(() => resolveTrack(kind, tokens), [kind, tokens]);
