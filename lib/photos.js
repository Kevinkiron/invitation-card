...
export const BUCKET = "invite-photos";
...
export function describeFile(file) {
  ...
  if (!TYPES.includes(file.type)) {
    return `${file.name || "That file"} is not an image I can use — JPEG, PNG or WebP please.`;
  }
...
export async function uploadPhoto(file, userId) {
  ...
  if (error) {
    ...
    if (/bucket/i.test(error.message || "")) {
      throw new Error(
        "Photo storage is not set up yet. Create a public bucket named " +
        `"${BUCKET}" in Supabase — the SQL is in supabase/schema.sql.`
      );
    }
